const Version = '2026-09-07 04:08:34';
...
const EDT_UA = 'v2rayN/edge' + 'tunnel (https://github.com/' + 特征码字典[1] + '/edge' + 'tunnel)';
...
async function 请求优选API(urls, 默认端口 = '443', 超时时间 = 3000) {
	...
	await Promise.allSettled(urls.map(async (url) => {
		...
		try {
			...
			const response = await fetch(urlWithoutHash, { signal: controller.signal, headers: { 'User-Agent': EDT_UA } });
